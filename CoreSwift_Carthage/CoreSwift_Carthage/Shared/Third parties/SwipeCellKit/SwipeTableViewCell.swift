...
extension SwipeTableViewCell: SwipeActionsViewDelegate {
    func swipeActionsView(_ swipeActionsView: SwipeActionsView, didSelect action: SwipeAction, actionButton: UIButton) {
        perform(action: action, actionButton: actionButton)
    }
    
    func perform(action: SwipeAction, actionButton: UIButton? = nil) {
        guard let actionsView = actionsView else { return }
        ...
        if action == actionsView.expandableAction, let expansionStyle = actionsView.options.expansionStyle {
            ...
            case .bounce:
                perform(action: action, actionButton: actionButton, hide: true)
            case .fill(let fillOption):
                performFillAction(action: action, fillOption: fillOption)
            }            
        } else {
            perform(action: action, actionButton: actionButton, hide: action.hidesWhenSelected)
        }
    }
    
    func perform(action: SwipeAction, actionButton: UIButton? = nil, hide: Bool) {
        ...
        action.handler?(action, actionButton, indexPath)
    }
    ...
    func performFillAction(action: SwipeAction, fillOption: SwipeExpansionStyle.FillOptions) {
        ...
        let invokeAction = {
            action.handler?(action, nil, indexPath)
