...
@IBDesignable open class BadgeSwift: UILabel {
  ...
  @IBInspectable open var badgeBorderWidth: CGFloat = 0 {
    didSet {
  ...
  @IBInspectable open var badgeBorderColor: UIColor = UIColor.white {
    didSet {
      invalidateIntrinsicContentSize()
    }
  }
  ...
  override open func draw(_ rect: CGRect) {
    let rectInset = rect.insetBy(dx: badgeBorderWidth/2, dy: badgeBorderWidth/2)
    ...
    if badgeBorderWidth > 0 {
      ...
      path?.lineWidth = badgeBorderWidth
      path?.stroke()
    }
  ...
  private func actualInsetsWithBorder() -> CGSize {
    return CGSize(
      width: insets.width + badgeBorderWidth,
      height: insets.height + badgeBorderWidth
    )
  }
